...
type ExtractValues<T> = T extends Record<string, infer U>
  ? U extends Record<string, unknown>
    ...
    : U
  : never;
type FirstTwoChars<T extends string> = T extends `${infer A}${infer B}${string}`
  ? `${A}${B}`
  : T;

export type GeoPosId = ExtractValues<GeoPosData>;
export type GeoPosSuggestion =
  | GeoPosId
  | FirstTwoChars<ExtractValues<GeoPosData>>;
...
export const geoPosName = (() => {
  ...
  for (const [key, value] of Object.entries(geoPosData)) {
    if (key.match(/沖縄県|北海道/)) {
      for (const [_key2, value2] of Object.entries(value)) {
